...
const BASE = 'https://mechauniverse.es';
...
function keyFor(model: GundamModel) { return `store:mechauniverse:v2:${(model.name||'').toLowerCase()}|${model.grade||''}`; }

export async function getMechaUniversePrice(model: GundamModel): Promise<{ price: number; currency: 'EUR'; url?: string } | null> {
