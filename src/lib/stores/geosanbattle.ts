  ...
  try {
    ...
    if (rj.ok) {
      ...
      if (price != null) {
        ...
        if (Number.isFinite(num)) return { price: num, currency: 'USD', url: `${BASE}/products/${handle}` };
      }
    }
  } catch { /* ignore */ }
  try {
    ...
    if (rjs.ok) {
      ...
      const priceRaw = v?.price ?? data?.price ?? data?.price_min ?? null;
      if (priceRaw != null) {
        let num = typeof priceRaw === 'string' ? parseFloat(priceRaw.replace(',', '.')) : Number(priceRaw);
        if (Number.isFinite(num)) {
          if (num >= 100 && num % 1 === 0) num = num/100;
          return { price: Math.round(num*100)/100, currency: 'USD', url: `${BASE}/products/${handle}` };
        }
      }
    }
  } catch { /* ignore */ }
  ...
  try {
    ...
    if (ld && ld[1]) {
      try {
        ...
        const currency = (offer?.priceCurrency || offer?.[0]?.priceCurrency || 'USD').toUpperCase() as 'USD'|'EUR';
    ...
    if (m) {
      const currency: 'USD'|'EUR' = m[1] === '€' ? 'EUR' : 'USD';
